...
\begin{document}
	...
	In order to regulate the transmissions held during SR-based opportunities, the transmission power is limited according to the OBSS/PD threshold used for detecting those opportunities. In case that $\text{OBSS/PD}_{\max} \geq \text{OBSS/PD} > \text{OBSS/PD}_{\min}$, the transmission power is unconstrained. Otherwise, the maximum allowed transmission power $\text{TX\_PWR}_{\max}$ is given by:
	\begin{equation}
	...
	In case of having detected an SR-based opportunity, an HE node will continue decreasing its backoff. Once the backoff counter is over and the node is about to transmit, a transmit power limitation is applied. In particular, the transmit power is adjusted according to \eqref{eq:power_restriction}. Finally, once the HE node finishes its SR-based transmission, it returns to the default sensing state, where channel access is scheduled according to the legacy CCA/CS threshold (given that more packets are waiting for transmission in the buffer).
	...
	\begin{table}[h]
		...
		\resizebox{\columnwidth}{!}{
			\begin{tabular}{c|l|l|}
				...
				\cline{2-3} 
				\multicolumn{1}{|c|}{} & Average num. of walls per meter $\overline{W}$              & 0.1467   \\
				\cline{2-3}
	...
	\begin{thebibliography}{00}
		...
		%\bibitem{wilhelmi2019tutorial} Wilhelmi, F., Barrachina-Mu\~noz, S., Bellalta, B., Cano, C. \& Selinis, I. (2019). ``Spatial Reuse in the IEEE 802.11ax: Analysis, Challenges and Opportunities." arXiv preprint arXiv:XXX, unpublished.
